...
설치하면 23개 커맨드를 쓸 수 있다. 외울 필요 없이 **흐름대로 따라가면 된다.**

### 전체 플로우

```
아이디어 → 검증 → 설계 검증 → 구현 → 품질 점검 → 코드 리뷰 → 배포 → 헬스체크 → 문서 갱신
```

| 단계 | 커맨드 | 설명 |
|------|--------|------|
| 1. 기획 | `/office-hours 아이디어` | 아이디어를 구조화하고, 진짜 필요한 건지 검증 |
| 2. 계획 검증 | `/plan-review` | 구현 전에 아키텍처, 실패 시나리오, 테스트 전략 점검 |
| 3. 디자인 검증 | `/design-check` | UI가 있으면 설계 점수화 (없으면 건너뜀) |
| 4. 구현 | `/feature 기능명` | 코드 작성 (서브에이전트가 알아서 구현) |
| 5. 품질 점검 | `/simplify` | 구현한 코드의 재사용성, 품질, 효율성 자가 점검 |
| 6. 코드 리뷰 | `/review` | 위험 패턴 10가지 자동 감지 (SQL injection, XSS 등) |
| 7. 배포 | `/ship` | 테스트 → 리뷰 → 버전 bump → PR 생성 원스톱 |
| 8. 헬스체크 | `/canary URL` | 배포 후 프로덕션 상태 확인 (이전 배포와 비교) |
| 9. 문서 갱신 | `/document-release` | README, CHANGELOG 자동 업데이트 |

> **자동 연결**: 4→5→6은 앞 단계가 끝나면 자동으로 다음이 실행된다. 배포(7)부터는 확인을 받는다.

### 버그 수정 플로우

```
버그 발견 → 원인 조사 → 수정 → 점검 → 리뷰 → 배포
```

| 단계 | 커맨드 | 설명 |
|------|--------|------|
| 1. 원인 조사 | `/investigate 증상` | 코드를 수정하지 않고 원인만 파악 |
| 2. 수정 | `/fix 증상` | 확정된 원인 기반으로 수정 |
| 3~6 | `/simplify` → `/review` → `/ship` | 위와 동일 |
...
```bash
# 1. 아이디어 정리 (선택)
claude "/office-hours 게시판 기능"

# 2. 구현 — 이것만 치면 된다
claude "/feature 게시판 목록 조회"

# 3. 자동으로 이어짐:
#    /simplify (품질 점검) → /review (코드 리뷰)

# 4. 배포
claude "/ship"

# 5. 확인
claude "/canary http://localhost:3000"
```

> 처음엔 `/feature`와 `/ship`만 알면 된다. 나머지는 쓰다 보면 자연스럽게 익힌다.
