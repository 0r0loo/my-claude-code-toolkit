...
- **FEAT**: `/plan-review` 프롬프트 추가 — 구현 전 계획 검증
  - 5차원 검증: 범위, 아키텍처, 데이터 흐름, 실패 시나리오, 테스트 전략
  - 이슈별 인터랙티브 합의 (하나씩 질문 → 선택지 → 합의)
  - 가치관 연동: "가장 단순한 버전인가?", "되돌릴 수 있는가?"
  - M/L 티어 작업에서 구현 전 자동 실행
- **FEAT**: `/design-check` 프롬프트 추가 — 구현 전 UI/UX 설계 검증
  - 6차원 점수화: 정보 계층, 인터랙션 상태, 레이아웃, 반응형, 접근성, 엣지 케이스
  - 각 차원에서 "10점이 되려면 뭘 해야 하는지" 구체적 제안
  - 7점 미만 차원만 개선 합의 (나머진 PASS)
  - UI가 포함된 작업에서만 자동 실행
- **UPDATE**: 파이프라인 확장
  - `/office-hours` → `/plan-review` → `/design-check` → `/feature` → `/simplify` → `/review` → `/ship`

## 1.15.0

### 가치관 + 자동 파이프라인 + /help-me

- **FEAT**: 가치관 섹션 추가 (CLAUDE.md 섹션 0)
  - 4원칙: 문제 먼저 / 가장 단순한 버전 / 증거 기반 / 되돌릴 수 있는가
  - 각 원칙에서 상황별 커맨드 자연 연결 (가치관 → 커맨드 추천)
- **FEAT**: 파이프라인 자동 연결 (섹션 6 업그레이드)
  - `/feature` → `/simplify` → `/review` → `/ship` → `/canary` → `/document-release` 자동 실행
  - 코드 작업은 자동, 배포/위험 작업은 사용자 확인
  - "자동 실행 그만" 시 추천 모드로 전환
- **FEAT**: `/help-me` 프롬프트 추가 — 커맨드 추천 + 자동 실행
  - 인자 없이: 21개 커맨드를 상황별 그룹으로 가이드 출력
  - 인자 있으면: 3단계 판단 (매칭 → 명확도 → 실행 or 질문)
  - 명확하면 바로 실행, 애매하면 2~3개 선택지
...
- **FEAT**: `/office-hours` 제품 진단 + 설계 추가
  - 진단 모드 (스타트업): 수요 검증, 현재 대안, 최소 쐐기 등 5가지 질문
  - 빌더 모드 (사이드 프로젝트): 창의적 협업, wow factor 탐색
  - 전제 도전 → 접근법 비교 → 디자인 문서 출력
