...
### Phase 7 — 수정 루프 (Fix Loop)

QA 리포트에서 NEEDS_FIX 판정이면, Critical/High 이슈를 순서대로 수정한다.

**이슈당 1사이클:**
1. **소스 코드 읽기** — 이슈 관련 컴포넌트/페이지 소스를 읽고 원인 파악
2. **수정** — 최소한의 변경으로 이슈 해결
3. **원자적 커밋** — 이슈 1개 = 커밋 1개 (`FIX: [ISSUE-001] 설명`)
4. **재검증** — 브라우저에서 같은 시나리오를 다시 테스트, 스크린샷 비교
5. **결과 기록** — before/after 스크린샷 + 수정 내용

**루프 종료 조건:**
- Critical/High 이슈가 모두 해결됨
- 또는 수정이 범위를 넘어섬 (구조적 변경 필요) → 사용자에게 보고하고 중단

**최종 보고:**
```
## Fix Summary
- 수정됨: [N]건 (ISSUE-001, ISSUE-003, ...)
- 미해결: [M]건 (ISSUE-002: 구조 변경 필요)
- Health Score: [이전] → [이후]
```

> Medium/Low 이슈는 수정하지 않는다. 보고만 한다.
> 수정 없이 보고만 원하면 `/qa-only`를 사용한다.
...
5. **Phase 1-6은 발견만** — 수정은 Phase 7에서. 리포트 전용은 `/qa-only`
