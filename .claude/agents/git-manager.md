...
### 절차
1. `git status`로 변경 파일 전체를 확인한다
2. `git diff`로 각 파일의 변경 내용을 확인한다
3. **변경사항을 논리적 단위로 그룹핑한다** (기능, 리팩터, 문서, 설정 등)
4. 그룹별로 관련 파일만 `git add <파일경로>` → 커밋을 반복한다
5. 각 커밋 후 `git status`로 남은 변경사항을 확인한다

### 커밋 분리 기준

여러 종류의 변경이 섞여있으면 **반드시 분리 커밋**한다.

| 분리 대상 | 예시 |
|-----------|------|
| 기능 A + 기능 B | 각각 별도 커밋 |
| 기능 + 리팩터 | `FEAT:` + `REFACTOR:` 분리 |
| 기능 + 문서 | `FEAT:` + `DOCS:` 분리 |
| 기능 + 설정 변경 | `FEAT:` + `CHORE:` 분리 |
| 기능 + 테스트 | 함께 커밋 OK (관련 테스트는 같은 커밋) |

> 판단이 어려우면 "이 커밋을 revert했을 때 의미 있는 단위인가?"로 판단한다.
